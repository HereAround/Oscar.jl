...
There can be subtle changes in the parent of results. In the following example we create the same polynomial twice: once as an element of a polynomial ring (via `/`) and once as an element of its field of fractions (via `//`). We can add these ring elements despite residing in different parents, thanks to our[promotion rules](https://nemocas.github.io/AbstractAlgebra.jl/stable/ring_interface/#Promotion-rules), but the result lives in the field of fractions. This may be unexpected at first, as the result is promoted to the larger parent.
